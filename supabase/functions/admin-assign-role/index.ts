...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!userId || !roleId) return fail("Missing userId or roleId", 400, "BAD_REQUEST");
    ...
    if (error) return fail(error.message, 400, "BAD_REQUEST", { userId, roleId });

    await logAuditEvent(adminClient, {
      actorId: caller.id,
      action: "admin_assign_role",
      entity: "user_roles",
      metadata: { user_id: userId, role_id: roleId },
    });
